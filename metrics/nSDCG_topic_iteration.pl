...
$usage = "usage: $0 qrels run_directory iteration-capacity [discounter(BQ)]";
...
print "runid,topic,iteration,nsdcg\n";
$topicCurrent = "-1";
my $ndcgTotal = 0;
for ($i = 0; $i <= $#run; $i++) {
...
sub topicDone {
  ...
  if (exists $seen{$topic}) {
    $sdcg = &dcg($runid, $K, $topic, $gains, $iterations);
    $topics++;
  }
  return $sdcg;
}
...
sub dcg {
 my ($runid, $k, $tid, $tmp_gains, $tmp_iterations) = @_;
 my @gain = @{$tmp_gains};
 ...
 for ($i = 0; $i <= ($k <= $#gain ? $k - 1 : $#gain); $i++) {
   if($iterations[$i] != $pre_iteration){
      ...
      my $msg_score = $score/ $idealGain;
      print "$runid,$tid,$pre_iteration,$msg_score\n";
 ...
 if($#gain >= 0){
   ...
   print "$runid,$tid,$pre_iteration,$score\n";
