...
static void
outputMeasures (
  struct rList *rl, int topics, int actualTopics, char *runid, int all
)
{
  int i=0;
  ...
  double mean_alpha_NDCG =0, mean_nERR_IA = 0;
  for (i = 0; i < topics; i++)
    {
      int per_iteration = -1; 
  ...
  if ((dir = opendir (argv[2])) != NULL) {
    ...
    while ((ent = readdir (dir)) != NULL) {
      if(strncmp(ent->d_name, ".", strlen(".")) != 0){
         char * file_path = concat(argv[2], ent->d_name); 
         //printf ("%s\n", ent->d_name);
         //printf ("%s\n", file_path);
         ...
         outputMeasures (rrl, rTopics, actualTopics, file_path, aFlag);
